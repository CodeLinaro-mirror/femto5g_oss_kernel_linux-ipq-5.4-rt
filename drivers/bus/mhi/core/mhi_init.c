...
{
	MHI_LOG("Time response: seq:0x%x local: %llu remote: %llu (ticks)\n",
				 ...
				 u64 local_time, u64 remote_time)
{
	...
	MHI_LOG("Time response: seq:0x%x local: %llu remote: %llu (us)\n",
			       ...
			       char *buf)
{
	...
	if (ret) {
		MHI_ERR("Failed to request time, seq:0x%x, ret:%d\n", seq, ret);
		return scnprintf(buf, PAGE_SIZE,
				 "Request failed or feature unsupported\n");
	}

	return scnprintf(buf, PAGE_SIZE,
			 "Requested time asynchronously with seq:0x%x\n", seq);
}
...
void mhi_create_sysfs(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret)
		MHI_ERR("mhi_create_sysfs sysfs_group failed\n");

	if (mhi_cntrl->mhi_tsync)
		ret = sysfs_create_group(&mhi_cntrl->mhi_dev->dev.kobj,
				   &mhi_tsync_group);
	if (ret)
		MHI_ERR("mhi_create_sysfs tsync_group failed\n");
}
		       ...
		       struct device_node *of_node)
{
	...
	mhi_cntrl->of_node_devices = of_find_node_by_name(of_node, "mhi_devices");
	if (!mhi_cntrl->of_node_devices)
		return -EINVAL;
